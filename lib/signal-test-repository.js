import { getAccessRequestByCode } from "./access-repository";
import { assertCampaignIsActive } from "./campaign-access";
import { createEvent, hasSupabase, restRequest, rpcRequest } from "./supabase-client";
import { getTestAccessCode } from "./runtime-config";
import { upsertPanelContactEngagementStatus } from "./panel-launch";

function parseScale(value) {
  if (value == null || value === "") {
    return null;
  }
  const parsed = Number.parseInt(String(value), 10);
  return Number.isFinite(parsed) ? parsed : null;
}

function averageScales(values) {
  const valid = values.filter((value) => Number.isFinite(value));
  if (!valid.length) {
    return null;
  }
  return Math.round(valid.reduce((sum, value) => sum + value, 0) / valid.length);
}

function mapReactionScore(value) {
  const map = {
    "Je veux voir ça": 5,
    "Je suis curieux·se": 4,
    Bof: 2,
    "Pas pour moi": 1,
  };
  return map[value] || null;
}

function mapReactionLabel(value) {
  const normalized = String(value || "").trim();
  return normalized || null;
}

function mapBinaryScore(value) {
  const normalized = String(value || "").trim().toLowerCase();

  if (normalized === "oui") {
    return 5;
  }
  if (normalized === "non") {
    return 1;
  }
  return null;
}

function mapRoomReadingScore(value) {
  const map = {
    "Trop long": 1,
    "Un peu long": 2,
    "Bien comme il faut": 4,
    "Rapide & fluide": 5,
  };
  return map[value] || null;
}

function buildTestResponseRow(enrolmentId, answers) {
  return {
    enrolment_id: enrolmentId,
    submitted_at: new Date().toISOString(),
    immediate_reaction_score: mapReactionScore(answers.reaction),
    promise_clarity_score: parseScale(answers.clarte),
    watch_intent_score: parseScale(answers.envie_regarder),
    emotional_intensity_score: parseScale(answers.intensite),
    mood_clarity_score: averageScales([
      parseScale(answers.clarte_univers),
      parseScale(answers.clarte_dramatique),
      parseScale(answers.clarte_emotionnel),
    ]),
    character_interest_score: parseScale(answers.interet_perso),
    character_projection_score: parseScale(answers.projection_perso),
    continue_intent_score: parseScale(answers.questions),
    recommendation_score: parseScale(answers.reco),
    talkability_score: parseScale(answers.conversation),
  };
}

function buildTestResponseV2Row(enrolmentId, answers) {
  return {
    ...buildTestResponseRow(enrolmentId, answers),
    curiosity_score: parseScale(answers.curiosite),
    world_clarity_score: parseScale(answers.clarte_univers),
    dramatic_clarity_score: parseScale(answers.clarte_dramatique),
    emotional_clarity_score: parseScale(answers.clarte_emotionnel),
  };
}

function buildTestResponseV3Row(enrolmentId, answers) {
  return {
    enrolment_id: enrolmentId,
    submitted_at: new Date().toISOString(),
    immediate_reaction_label: mapReactionLabel(answers.reaction),
    hook_discovery_score: parseScale(answers.envie_poursuivre),
    curiosity_score: parseScale(answers.curiosite),
    promise_clarity_score: parseScale(answers.clarte),
    world_clarity_score: parseScale(answers.clarte_univers),
    dramatic_clarity_score: parseScale(answers.clarte_dramatique),
    emotional_clarity_score: parseScale(answers.clarte_emotionnel),
    felt_something_score: parseScale(answers.ressenti),
    atmosphere_stay_score: parseScale(answers.ambiance_ton),
    character_intrigue_score: parseScale(answers.intrigue_perso),
    character_fate_interest_score: parseScale(answers.destin_perso),
    continue_intent_score: parseScale(answers.questions),
    watch_intent_score: parseScale(answers.envie_episode),
    recommendation_score: parseScale(answers.reco),
    talkability_score: parseScale(answers.conversation),
  };
}

function buildRoomFeedbackRow(enrolmentId, answers) {
  return {
    enrolment_id: enrolmentId,
    submitted_at: new Date().toISOString(),
    reading_fluidity_score: mapRoomReadingScore(answers.room_lecture),
    material_sufficiency_score: mapBinaryScore(answers.room_suffisant),
    usefulness_score: parseScale(answers.room_utilite),
    experience_quality_score: parseScale(answers.room_xp),
    duration_relevance_score: mapRoomReadingScore(answers.room_duree),
    device_recommendation_score: parseScale(answers.room_reco),
    free_comment: answers.room_plus || null,
  };
}

function buildRoomFeedbackV3Row(enrolmentId, answers) {
  return {
    enrolment_id: enrolmentId,
    submitted_at: new Date().toISOString(),
    reading_fluidity_score: mapRoomReadingScore(answers.room_lecture),
    material_sufficiency_score: mapBinaryScore(answers.room_suffisant),
    duration_relevance_score: mapRoomReadingScore(answers.room_duree),
    free_comment: answers.room_plus || null,
  };
}

function buildVerbatimPayload(answers) {
  const rows = [];

  const pushRow = (questionCode, value) => {
    if (!value) {
      return;
    }

    const text = Array.isArray(value) ? value.join(", ") : String(value).trim();
    if (!text) {
      return;
    }

    rows.push({
      question_code: questionCode,
      verbatim_text: text,
    });
  };

  pushRow("hook_accroche", answers.accroche);
  pushRow("feel_verbatim", answers.feel_verbatim);
  pushRow("feel_emotion", answers.emotionAutre || answers.emotion);
  pushRow("care_attachement", answers.attachement);
  pushRow("care_personnages", answers.care_perso);
  pushRow("care_pourquoi", answers.care_pourquoi);
  pushRow("continue_raisons", answers.continue_raison);
  pushRow("continue_decroche", answers.decroche);
  pushRow("continue_binge", answers.binge);
  pushRow("continue_drop", answers.drop);
  pushRow("share_cible", answers.cible);
  pushRow("share_pitch", answers.pitch_mots);

  return rows;
}

function buildVerbatimPayloadV3(answers) {
  const rows = [];

  const pushRow = (questionCode, value) => {
    if (!value) {
      return;
    }

    const text = Array.isArray(value) ? value.join(", ") : String(value).trim();
    if (!text) {
      return;
    }

    rows.push({
      question_code: questionCode,
      verbatim_text: text,
    });
  };

  pushRow("feel_emotion", answers.emotion);
  pushRow("care_personnages", answers.care_perso);
  pushRow("care_pourquoi", answers.care_pourquoi);
  pushRow("continue_decroche", answers.decroche);
  pushRow("continue_moteur_frein", answers.continue_moteur_frein);
  pushRow("share_pitch", answers.pitch_mots);

  return rows;
}

function buildVerbatims(testResponseId, answers) {
  return buildVerbatimPayload(answers).map((row) => ({
    test_response_id: testResponseId,
    ...row,
  }));
}

function isSignalV2Access(accessRecord) {
  return accessRecord?.projectVersion === "maree_noire_v2" && accessRecord?.formVersion === "signal_v2";
}

function isSignalV3Access(accessRecord) {
  return accessRecord?.projectVersion === "maree_noire_v2" && accessRecord?.formVersion === "signal_v3";
}

async function submitSignalTestLegacy(accessRecord, answers) {
  const enrolmentId = accessRecord.id;
  const testResponseBody = buildTestResponseRow(enrolmentId, answers || {});

  const testResponseRows = await restRequest({
    schema: "signal",
    table: "test_responses",
    query: "?on_conflict=enrolment_id",
    method: "POST",
    prefer: "resolution=merge-duplicates,return=representation",
    body: testResponseBody,
  });

  const testResponse = testResponseRows[0];
  const verbatims = buildVerbatims(testResponse.id, answers || {});

  await restRequest({
    schema: "signal",
    table: "test_verbatims",
    query: `?test_response_id=eq.${testResponse.id}`,
    method: "DELETE",
    prefer: "return=representation",
  });

  if (verbatims.length) {
    await restRequest({
      schema: "signal",
      table: "test_verbatims",
      method: "POST",
      body: verbatims,
    });
  }

  const roomFeedbackRows = await restRequest({
    schema: "signal",
    table: "room_feedback",
    query: "?on_conflict=enrolment_id",
    method: "POST",
    prefer: "resolution=merge-duplicates,return=representation",
    body: buildRoomFeedbackRow(enrolmentId, answers || {}),
  });

  await restRequest({
    schema: "ops",
    table: "enrolments",
    query: `?id=eq.${enrolmentId}`,
    method: "PATCH",
    body: {
      completed_at: new Date().toISOString(),
      status: "completed",
    },
  });

  await createEvent({
    contactId: accessRecord.contactId,
    campaignId: accessRecord.campaignId,
    enrolmentId,
    eventType: "test_submitted",
    payload: {
      test_response_id: testResponse.id,
      room_feedback_id: roomFeedbackRows?.[0]?.id || null,
    },
  });

  await upsertPanelContactEngagementStatus(accessRecord.contactId, {
    statut_engagement: "actif",
    derniere_action: "test_complete",
    date_derniere_action: new Date().toISOString(),
    a_relancer: false,
    parcours_complet: true,
  });

  return {
    ok: true,
    enrolmentId,
    testResponseId: testResponse.id,
  };
}

async function submitSignalTestV2Atomic(accessRecord, answers) {
  const enrolmentId = accessRecord.id;
  const result = await rpcRequest({
    schema: "signal",
    fn: "submit_signal_test_v2",
    body: {
      p_access_code: accessRecord.accessCode,
      p_test_response: buildTestResponseV2Row(enrolmentId, answers || {}),
      p_room_feedback: buildRoomFeedbackRow(enrolmentId, answers || {}),
      p_verbatims: buildVerbatimPayload(answers || {}),
    },
  });
  const submission = Array.isArray(result) ? result[0] : result;

  await createEvent({
    contactId: submission?.contact_id || accessRecord.contactId,
    campaignId: submission?.campaign_id || accessRecord.campaignId,
    enrolmentId,
    eventType: "test_submitted",
    payload: {
      test_response_id: submission?.test_response_id || null,
      room_feedback_id: submission?.room_feedback_id || null,
      mode: "atomic_v2",
    },
  });

  return {
    ok: true,
    enrolmentId,
    testResponseId: submission?.test_response_id || null,
  };
}

async function submitSignalTestV3Atomic(accessRecord, answers) {
  const enrolmentId = accessRecord.id;
  const result = await rpcRequest({
    schema: "signal",
    fn: "submit_signal_test_v3",
    body: {
      p_access_code: accessRecord.accessCode,
      p_test_response: buildTestResponseV3Row(enrolmentId, answers || {}),
      p_room_feedback: buildRoomFeedbackV3Row(enrolmentId, answers || {}),
      p_verbatims: buildVerbatimPayloadV3(answers || {}),
    },
  });
  const submission = Array.isArray(result) ? result[0] : result;

  await createEvent({
    contactId: submission?.contact_id || accessRecord.contactId,
    campaignId: submission?.campaign_id || accessRecord.campaignId,
    enrolmentId,
    eventType: "test_submitted",
    payload: {
      test_response_id: submission?.test_response_id || null,
      room_feedback_id: submission?.room_feedback_id || null,
      mode: "atomic_v3",
    },
  });

  return {
    ok: true,
    enrolmentId,
    testResponseId: submission?.test_response_id || null,
  };
}

export async function submitSignalTest({ accessCode = "", answers }) {
  if (!hasSupabase()) {
    return { ok: true, mode: "local" };
  }

  const resolvedAccessCode = String(accessCode || getTestAccessCode() || "").trim().toUpperCase();

  if (!resolvedAccessCode) {
    throw new Error("missing_access_code");
  }

  const accessRecord = await getAccessRequestByCode(resolvedAccessCode);
  if (!accessRecord) {
    throw new Error("unknown_access");
  }

  assertCampaignIsActive(accessRecord);

  if (isSignalV3Access(accessRecord)) {
    return submitSignalTestV3Atomic(accessRecord, answers);
  }

  if (isSignalV2Access(accessRecord)) {
    return submitSignalTestV2Atomic(accessRecord, answers);
  }

  return submitSignalTestLegacy(accessRecord, answers);
}
