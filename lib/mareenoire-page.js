...
const EDITORIAL_FILENAME = "Maree_Noire_version_consolidee_corrigee.md";
const BONUS_CHARACTER_NAMES = new Set(["Iroise", "Soaz", "Camille", "Vera", "Vikram", "Lenn", "Le Braz", "Noé"]);
...
function stripWrappingMarkdown(value, marker) {
  const text = String(value || "").trim();
  return text.startsWith(marker) && text.endsWith(marker) ? text.slice(marker.length, -marker.length).trim() : text;
}

function parseMareeNoireEditorial(markdown) {
  const chapters = [];
  const characters = [];
  let current = null;
  let inBonus = false;
  ...
  for (const rawLine of String(markdown || "").split(/\r?\n/)) {
    ...
    if (line.toUpperCase().startsWith("BONUS - FICHES PERSONNAGES")) {
      inBonus = true;
      current = null;
      continue;
    }

    const chapterMatch = line.match(/^\*\*(\d{2})\s+—\s+(.+?)\*\*$/);
    if (!inBonus && chapterMatch) {
      current = {
    ...
    const characterName = stripWrappingMarkdown(line, "**");
    if (inBonus && BONUS_CHARACTER_NAMES.has(characterName)) {
      current = {
        name: characterName,
        meta: "",
        paragraphs: [],
    ...
    if (inBonus && !current.meta && /^\*[^*].*[^*]\*$/.test(line)) {
      current.meta = stripWrappingMarkdown(line, "*");
      continue;
    }
...
function getEditorialContent() {
  try {
    const source = fs.readFileSync(path.join(process.cwd(), EDITORIAL_FILENAME), "utf8");
    return parseMareeNoireEditorial(source);
  } catch {
    return { chapters: [], characters: [] };
  }
}
...
function getExperienceImages(page) {
  const featureByName = new Map((page?.characterFeatures ?? []).map((item) => [stripHtml(item.nameHtml), item.imageSrc]));
...
export function getMareeNoirePageData() {
  ...
  return {
    ...
    audio: {
      kicker: "",
      title: "Écouter les fragments",
      description:
        "Six segments audio courts et immersifs accompagnent les fragments essentiels de Marée Noire.",
      durationLabel: "Fragments courts",
      chapterCountLabel: "6 fragments",
      note: "",
      stickyLabel: "Marée Noire",
      ctaLabel: "Écouter les fragments",
      pauseLabel: "Mettre en pause",
      resumeLabel: "Reprendre l'écoute",
      jumpLabel: "Voir les chapitres",
      compactJumpLabel: "Chapitres",
      src: "",
      mimeType: "audio/mpeg",
      durationSeconds: null,
      chapters: [
        { label: "01", title: "Le retour de Noé", time: 0 },
        { label: "02", title: "Deux mères", time: 0 },
        { label: "03", title: "La ville / La digue", time: 0 },
        { label: "04", title: "Ceux qui cherchent", time: 0 },
        { label: "05", title: "Les Veilleurs", time: 0 },
        { label: "06", title: "Ce que la mer réclame", time: 0 },
      ],
    },
    beforeTest: [
      "Regarder le teaser dans de bonnes conditions.",
      "Traverser les six fragments essentiels, chacun avec son court segment audio immersif.",
      "Ouvrir les bonus seulement si vous voulez aller plus loin.",
      "Donner votre signal final lorsque le parcours vous semble clair.",
    ],
    expectation:
    ...
    experienceGuide: {
      ...
      items: [
        ...
        "Nous conseillons vivement de faire le test sur ordinateur, tablette ou grand mobile pour une meilleure expérience.",
      ],
    },
    intro: {
      kicker: "Session 1",
      ...
      notes: [
        "Regarder le teaser dans de bonnes conditions.",
        "Traverser les six fragments essentiels avant de répondre.",
        "Écouter les courts segments audio immersifs intégrés au parcours si vous le souhaitez.",
        "Ouvrir les bonus uniquement pour approfondir les personnages, les arènes et l'intention.",
      ...
      estimatedDuration: "Durée estimée : 7-10 min",
      expectation:
    ...
    pitch: {
      ...
      paragraphs: [
        ...
        "Alors que tous cherche à transformer l'inexplicable en récit, autour de la digue, au rythme des marées, un phénomène étrange s'installe, insidieusement.",
      ],
...
export function getMareeNoireExperiencePageData() {
  ...
  return {
    ...
    hero: page.hero,
    teaser: page.teaser,
    experienceGuide: page.experienceGuide,
    footerLinks: page.footerLinks,
    editorial: page.editorial,
    experienceImages: getExperienceImages(page),
  };
}
