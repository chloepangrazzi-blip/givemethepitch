...
function renderUnsubscribeFooter(unsubscribeUrl) {
  ...
  return `
    ...
    <p style="margin:0;${solidTextStyles(
      MUTED
    )};font-family:${FONT_STACK};font-size:12px;line-height:19px;font-weight:300;text-align:center;">
      Si vous ne souhaitez plus recevoir ces messages de campagne, vous pouvez
      <a href="${escapeHtml(unsubscribeUrl)}" style="${solidTextStyles(
    MINT
  )};text-decoration:underline;">vous désinscrire ici</a>.
    </p>
  `;
}
...
export function buildPanelLaunchStartReminderEmailHtml({ fullName, theRoomUrl, unsubscribeUrl }) {
  const firstName = encodeEmailText(getFirstName(fullName));
...
export function buildPanelLaunchStartReminderEmailText({ fullName, theRoomUrl, unsubscribeUrl }) {
  const firstName = getFirstName(fullName);
...
export function buildPanelLaunchEmailHtml({ fullName, theRoomUrl, unsubscribeUrl }) {
  const firstName = encodeEmailText(getFirstName(fullName));
...
export function buildPanelLaunchEmailText({ fullName, theRoomUrl, unsubscribeUrl }) {
  const firstName = getFirstName(fullName);
...
export function buildPanelCampaignClosingEmailHtml({ fullName, voteOptions = [], unsubscribeUrl }) {
  const firstName = encodeEmailText(getFirstName(fullName));
...
export function buildPanelCampaignClosingEmailText({ fullName, voteOptions = [], unsubscribeUrl }) {
  const firstName = getFirstName(fullName);
...
export async function sendPanelLaunchInviteEmail({
  ...
  return sendResendEmail({
    ...
    subject: "THE ROOM × Marée Noire (annule et remplace le précédent)",
    html: buildPanelLaunchEmailHtml({ fullName, theRoomUrl, unsubscribeUrl }),
    text: buildPanelLaunchEmailText({ fullName, theRoomUrl, unsubscribeUrl }),
  });
}
...
export async function sendPanelLaunchStartReminderEmail({
  ...
  return sendResendEmail({
    ...
    subject: "Il vous reste 48h pour découvrir Marée Noire",
    html: buildPanelLaunchStartReminderEmailHtml({ fullName, theRoomUrl, unsubscribeUrl }),
    text: buildPanelLaunchStartReminderEmailText({ fullName, theRoomUrl, unsubscribeUrl }),
  });
}
...
export async function sendPanelLaunchAccessReminderEmail({
  ...
  return sendResendEmail({
    ...
    subject: "Votre accès THE ROOM est toujours actif",
...
export async function sendPanelCampaignClosingEmail({
  ...
  return sendResendEmail({
    ...
    subject: "Vote pour le prochain pitch de THE ROOM",
    html: buildPanelCampaignClosingEmailHtml({ fullName, voteOptions, unsubscribeUrl }),
    text: buildPanelCampaignClosingEmailText({ fullName, voteOptions, unsubscribeUrl }),
  });
}
