...
export function buildPanelLaunchEmailHtml({ fullName, theRoomUrl }) {
  ...
  const bodyContent = `
    ...
    ${renderCopyParagraph(
      "Le dossier complet est disponible en lecture et en audio. Le parcours dure environ 30 minutes. Il débute par un premier questionnaire consacré à vos habitudes de visionnage, se poursuit par la lecture de MARÉE NOIRE, puis s'achève par un second questionnaire sur le projet.",
      { marginBottom: 18 }
    )}
    ${renderCopyParagraph(
