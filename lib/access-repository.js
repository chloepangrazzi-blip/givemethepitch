...
import { assertCampaignIsActive, getCampaignAccessError } from "./campaign-access";
...
async function ensureDefaultCampaign(projectId) {
  const activeCampaign = await selectOne(
    "ops",
    "campaigns",
  ...
  const latestCampaign = await selectOne(
    "ops",
    "campaigns",
    `?project_id=eq.${projectId}&campaign_type=eq.${encodeURIComponent(
      DEFAULT_CAMPAIGN.campaign_type
  ...
  if (latestCampaign) {
    throw new Error(getCampaignAccessError(latestCampaign.status));
  }
...
function mapEnrolmentRecord({ enrolment, contact, campaign, project }) {
  return {
    ...
    campaignCode: campaign?.campagne_code || null,
    campaignStatus: campaign?.status || null,
    contactId: contact?.id || enrolment.contact_id,
    projectCode: project?.projet_code || null,
...
export async function ensureTestAccessRequest(accessCode = getTestAccessCode()) {
  ...
  if (existing) {
    assertCampaignIsActive(existing);
    return existing;
  }
